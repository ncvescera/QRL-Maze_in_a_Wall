...
class Maze:
    def __init__(self, n, m):
        ...
        self.actions = CommonElements.actions()
    ...
    def check_move(self, pos: tuple[int, int], action: str) -> tuple[bool, tuple[int, int]]:
        ...
        if action == self.actions["up"]:
            if (x-1) >= 0 and self.maze[(x-1)][y] != self.items["wall"]:
                to_return = True
                new_pos = ((x - 1), y)
        elif action == self.actions["down"]:
            if (x+1) < self.n and self.maze[(x+1)][y] != self.items["wall"]:
                to_return = True
                new_pos = ((x + 1), y)
        elif action == self.actions["right"]:
            if (y+1) < self.m and self.maze[x][(y+1)] != self.items["wall"]:
                to_return = True
                new_pos = (x, (y + 1))
        elif action == self.actions["left"]:
            if (y-1) >= 0 and self.maze[x][(y-1)] != self.items["wall"]:
                to_return = True
                new_pos = (x, (y - 1))
        else:
